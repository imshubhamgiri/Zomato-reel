...
const BottomNav = () => {
  const { user, isAuthenticated, showLoginModal, setShowLoginModal } = useAppContext();
  ...
  const navitems =[
        ...
        {path:user?.userType === 'partner' ? '/partner/profile' : '/user/profile' ,label:'Profile', icon:User, requiresAuth: true},
    ]
  return (
    <>
        ...
        <div className="flex items-center justify-around gap-1">
          {navitems.map((item) => (
            <NavLink 
              ...
              className={({ isActive }) => `group flex min-w-14 flex-col items-center gap-0.5 rounded-2xl px-3 py-2 text-[10px] font-medium tracking-tight transition-all duration-200
